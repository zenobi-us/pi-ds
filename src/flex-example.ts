...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("example-grid", {
		...
		handler: async (_args, ctx) => {
			...
			ctx.ui.setWidget("layout-demo", (tui, theme) => {
				...
				container.addChild(new Text(theme.bold(theme.fg("accent", "Grid Layout (Equal Widths)")), 0, 1));
	...
	pi.registerCommand("example-flex-fill", {
		...
		handler: async (_args, ctx) => {
			...
			ctx.ui.setWidget("layout-demo", (tui, theme) => {
				const container = new Container();
				container.addChild(new Text(theme.bold(theme.fg("accent", "Flex Fill Mode (Evenly Fill)")), 0, 1));
				...
				container.addChild(new Text(theme.fg("dim", "Each starts at min width, then shares extra space"), 0, 0));
	...
	pi.registerCommand("example-flex-wrap", {
		...
		handler: async (_args, ctx) => {
			...
			ctx.ui.setWidget("layout-demo", (tui, theme) => {
				const container = new Container();
				container.addChild(new Text(theme.bold(theme.fg("accent", "Flex Wrap Mode (Flow Layout)")), 0, 1));
				...
				const tags = [
					...
					"Rust", "Go", "Docker", "Kubernetes", "AWS", "Git"
				];
				...
				container.addChild(new Text(theme.fg("dim", "Tags wrap to next line when they don't fit"), 0, 0));
				container.addChild(flex);
				return container;
			});
	...
	pi.registerCommand("example-grid-vs-flex", {
		...
		handler: async (_args, ctx) => {
			...
			ctx.ui.setWidget("layout-demo", (tui, theme) => {
				...
				// Flex wrap example
				container.addChild(new Text(theme.bold(theme.fg("accent", "Flex Wrap (Natural Widths)")), 0, 1));
				const flex = new Flex({ mode: "wrap", spacing: 2 });
	...
	pi.registerCommand("example-dashboard", {
		...
		handler: async (_args, ctx) => {
			...
			ctx.ui.setWidget("layout-demo", (tui, theme) => {
				...
				for (const tag of tags) {
					tagsRow.addChild(sized(
						new Text(theme.fg("accent", `[${tag}]`), 0, 0),
						tag.length + 2
					));
				}
	...
	pi.registerCommand("example-responsive", {
		...
		handler: async (_args, ctx) => {
			...
			ctx.ui.setWidget("layout-demo", (tui, theme) => {
				...
				container.addChild(grid);
				container.addChild(new Text(theme.fg("dim", "Falls back to vertical when too narrow"), 0, 1));
				...
				for (const item of items) {
					flex.addChild(sized(
						new Text(theme.fg("text", item), 0, 0),
						item.length + 2
					));
				}
				container.addChild(flex);
