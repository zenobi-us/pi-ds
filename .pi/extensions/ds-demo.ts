...
export class UISimulator extends Container implements Component {
  ...
  private createInteractiveSection(): Container {
    ...
    const headerText = new Text(th.bold(th.fg('accent', '🎯 Interactive Elements')), 0, 0);
    ...
    linkBtn.addChild(new Text(th.fg('accent', '→ ') + th.fg('mdLink', 'View Documentation'), 0, 0));
  ...
  private createButton(label: string, fgColor: string, bgColor: string): Container {
    const th = this.theme;
    const container = new Container();

    const box = new Box(1, 0, (s) => th.bg(bgColor, s));
    const text = new Text(' ' + th.fg(fgColor, th.bold(label)) + ' ', 0, 0);
    box.addChild(text);
  ...
  private createMessageSection(): Container {
    ...
    // Section header
    const header = new Box(1, 1, (s) => th.bg('userMessageBg', s));
    const headerText = new Text(th.bold(th.fg('accent', '💬 Message Bubbles')), 0, 0);
    header.addChild(headerText);
    container.addChild(header);
    ...
    const thinkingText = new Text(th.fg('dim', '● Thinking... (analyzing request)'), 0, 0);
  ...
  private createCodeSection(): Container {
    ...
    // Section header
    const header = new Box(1, 1, (s) => th.bg('userMessageBg', s));
    const headerText = new Text(th.bold(th.fg('accent', '💻 Code Blocks')), 0, 0);
    header.addChild(headerText);
    container.addChild(header);
    ...
    const line3 = new Text(th.fg('syntaxPunctuation', '}'), 0, 0);

    const line4 = new Text('', 0, 0);

    const line5 = new Text(th.fg('syntaxComment', '// Call the function'), 0, 0);
  ...
  private createAlertSection(): Container {
    ...
    // Section header
    const header = new Box(1, 1, (s) => th.bg('userMessageBg', s));
    const headerText = new Text(th.bold(th.fg('accent', '⚡ Alerts & Status')), 0, 0);
    header.addChild(headerText);
    container.addChild(header);
    ...
    const warningAlert = new Alert(th, 'warning', 'Cannot be undone');
    grid.addChild(warningAlert);

    // Error Alert using Alert component
    const errorAlert = new Alert(th, 'error', 'Connection failed', {
      padding: 2,
    ...
    const infoAlert = new Alert(th, 'info', 'New updates available', {
  ...
  private createFormSection(): Container {
    ...
    // Section header
    const header = new Box(1, 1, (s) => th.bg('userMessageBg', s));
    const headerText = new Text(th.bold(th.fg('accent', '📝 Form Elements')), 0, 0);
    header.addChild(headerText);
    container.addChild(header);
    ...
    const inputText = new Text(' ' + th.fg('text', 'john_doe') + th.fg('dim', '_') + ' ', 0, 0);
    ...
    const disabledText = new Text(' ' + th.fg('dim', 'john@example.com') + ' ', 0, 0);
    ...
    const checkbox1 = new Text(th.fg('accent', '☑') + ' ' + th.fg('text', 'Remember me'), 0, 0);
    ...
    const validText = new Text(' ' + th.fg('text', '••••••••') + ' ', 0, 0);
    validField.addChild(validText);
    const validHint = new Text(th.fg('success', '✓ ') + th.fg('dim', 'Strong password'), 0, 0);
